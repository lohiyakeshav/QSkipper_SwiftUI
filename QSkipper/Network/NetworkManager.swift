...
class SimpleNetworkManager {
    ...
    // Add function to handle dictionary responses
    func makeRequest(url: String, method: String = "GET", body: Data? = nil) async throws -> [String: Any] {
        ...
        for attempt in 1...maxRetries {
            do {
                ...
                do {
                    print("📥 SimpleNetworkManager: Parsing JSON response...")
                    if let responseText = String(data: data, encoding: .utf8) {
                        print("📥 Response data: \(responseText)")
                    }
                    
                    guard let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                        throw SimpleNetworkError.decodingFailed(NSError(domain: "JSONParsing", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to parse JSON as dictionary"]))
                    }
                    
                    return jsonObject
                } catch {
                    print("❌ SimpleNetworkManager: JSON parsing error: \(error)")
